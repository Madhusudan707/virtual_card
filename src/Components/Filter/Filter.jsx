  ...
  return (
    <>
      <button
        className="shadow-sm"
        onClick={() => {
          setIsShowFilterMenu((prev) => !prev);
        }}
      >
        <i className="fas fa-sort-amount-down-alt  p-2">&nbsp;Filter</i>
      </button>
      <div
        className={`flex flex-col w-64 absolute right-4 z-10 border bg-white p-2 ${
          isShowFilterMenu ? "" : "hidden "
        }`}
      >
        <div className="flex mb-4 items-center">
          <i
            className="fas fa-times absolute right-4 cursor-pointer"
            onClick={() => {
              setIsShowFilterMenu((prev) => !prev);
            }}
          ></i>
